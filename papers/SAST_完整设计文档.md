...
### 1. 核心问题：不同分量需要不同的挤压策略

同步压缩变换（SST）包含两个独立步骤：(1) IF 估计——判断能量应该往哪里挤；(2) 挤压——把能量从原始位置挪到 IF 指向的位置。

传统 SST 的控制点在于步骤 (1)：为不同 bin 选择不同阶数的 IF 估计。Colominas & Meignen (2025) 的核心发现是**不同 TF 位置的最优 IF 阶数不同**——靠近瞬时频率的 bin 高阶有益，远离的 bin 高阶放大噪声。

MSST（Yu et al. 2019）使用不同的 IF 精化机制：`omega_{k+1}[η,b] = omega_k[omega_k[η,b], b]`——沿频率轴向能量集中的方向迭代查找，而非计算高阶导数。这个操作对所有 bin 都是安全的：谐波 bin 快速收敛到正确值，噪声 bin 也是向能量集中方向走，不会发散。

**因此 SAST 将控制点从步骤 (1) 移到步骤 (2)。** 所有 bin 统一使用 MSST 5 次查找后的最优 IF（omega_5），GAT 控制挤压动作本身：挤多宽（核宽 σ）、挤几轮（迭代次数 λ）。

```
传统 SST + Colominas 论文: 为每个 bin 选 IF 阶数 → "用几阶的 IF 估计？"
SAST:                    所有 bin 用最优 IF (omega_5) → "该不该挤？挤几轮？"
```

### 2. 水泵水轮机的三个分量正好对应三种挤压需求

我们的信号中存在三类物理性质完全不同的频率分量。它们需要不同的挤压策略，而非不同的 IF 阶数：

| 分量 | 频段 | 信号特征 | σ (核宽) | λ (轮数) | 理由 |
|------|:---:|------|:---:|:---:|------|
| **2xBPF** | 90-105 Hz | 纯谐波, IF 极度稳定 | σ_min (~0.5 bin) | **λ=1** | 窄核一轮已到位 |
| **BPF** | 42-55 Hz | FM 调制, IF 漂移 ~15 Hz | 中 (~4 bin) | **λ=3~5** | 中核适度保留调制, 多轮追 chirp |
| **LOW_FREQ** | 2-25 Hz | 涡带宽带, 本征带宽 ~10-15 Hz | σ_max (~15 bin), 宽核 | **λ=0** | 涡带的本征带宽是真实水力脉动, 不是测量模糊。窄核会把分布在 2-25 Hz 的能量强行集中到虚假的单一频率。宽核使挤压核宽度覆盖本征带宽, 能量原样保留 |

**实验验证**（MSST 不同 IF 精化次数的 Rényi 熵, T=2000）:

```
合成信号:
  1 次精化: R_LF=11.07  R_BPF=11.72  R_2x=11.08
  5 次精化: R_LF=10.99  R_BPF=11.00  R_2x=11.02

真实信号 (Class 2):
  1 次精化: R_LF=13.15  R_BPF=13.08  R_2x=11.54
  5 次精化: R_LF=11.27  R_BPF=10.55  R_2x=11.03
```

Rényi 熵在所有分量上都随精化次数升高而下降——纯数据驱动（如 Colominas 论文的坐标下降）会把所有 bin 推向最高精化次数。但 LOW_FREQ 的 Rényi 下降是以"宽带能量被挤成假细线"为代价的——Rényi 看不到物理合理性。这正是需要物理图的原因。

### 3. 现有方法的局限

**固定策略 SST**: 全频段统一挤压——对 2xBPF 浪费计算，对 LOW_FREQ 强行挤压破坏信号。

**自适应阶 SST** (Colominas & Meignen, 2025): 做到了逐 bin 选择 IF 阶数，但针对的是传统 SST（高阶导数放大噪声，所以噪声 bin 需降阶）。对 MSST 场景（lookup 无害）是过度设计。且使用坐标下降——穷举+不可微+不能泛化+不能注入先验。

### 4. SAST 的方案

将不可微的离散决策（挤不挤、挤几轮）转化为神经网络可以处理的连续回归：

```
GAT 输出 w_i ∈ (0,1)  ← 连续, 可微

w_i → σ_i = σ_min + (1-w_i) × Δσ  ← 核宽 (连续, 可微, 训练用)
w_i → λ_i = round(w_i × N_max)     ← 挤压轮数 (离散, 推理用)
w_i → ridge_factor                  ← bin 级参与因子 (推理用)
```

训练时梯度通过 σ_i → w_i → GAT 回传。推理时 λ_i 直接继承 w_i 的排序。所有 bin 统一使用 omega_5（最优 IF）。详见 §18。
...
分类器 (GlobalAvgPool → MLP) 实际依赖的特征:
  R_LF  = sum(|TFR|² in 2-25 Hz)    ← 低频能量占比
...
│      lambda_sqz, ridge_factor: "每个分量挤几轮, 每个 bin 参与多少"     │
...
|------|:---:|------|:---:|:---:|:---:|
| LOW_FREQ | 2-25 Hz | HYDRAULIC | 0.30 | 10 Hz | 0.50 |
...
**对 SAST 的作用**: MSST 提供 STFT（挤压源数据）和 omega_final + omegas（GAT 的节点特征和逐 bin 挤压控制）。
...
所有 bin 统一使用 omega_5（5 次 lookup 的最优 IF）
...
#### 24.6 lambda_sqz + ridge_factor：挤压参与度（仅推理）

lambda_sqz ∈ {0..N_max} 每节点挤压轮数，ridge_factor ∈ [0,1] 逐 bin 参与因子。

**诊断用途**: 验证挤压轮数分配的物理合理性（2xBPF 一轮到位, BPF 多轮追 chirp, LOW_FREQ 零轮），以及同节点内脊线 bin vs 边缘 bin 的参与差异。
...
|   分量信任度时序   |   原型注意力时序   |
+-------------------+-------------------+
| 5. gate_edge(t)   | 6. ridge_factor 图  |  <- "依据是什么"
|   物理边门控时序   |   逐 bin 参与因子   |
+-------------------+-------------------+
```
