...
| 分量 | 频段 | 信号特征 | 应挤几轮 | 理由 |
|------|:---:|------|:---:|------|
| **2xBPF** | 90-105 Hz | 纯谐波, IF 极度稳定 | **λ=1** | 一轮挤压已到位 |
| **BPF** | 42-55 Hz | FM 调制, IF 漂移 ~15 Hz | **λ=3~5** | 多轮挤压→每轮重估 IF→逐步追 chirp |
| **LOW_FREQ** | 8-20 Hz | 宽带非稳态, 无明确定义的 IF | **λ=0** | 不应挤压——IF 指向的是假质心 |
